...
-- ============================================================================
create
or replace function public.mark_goal_done_on_proof () returns trigger language plpgsql
set
...
after insert on public.proofs for each row
...
create policy "view own and group proofs" on public.proofs for
select
...
create policy "delete own proof" on public.proofs for delete to authenticated using (
  user_id = (
    select
      auth.uid ()
  )
