...
create policy "view own and group goals" on public.goals for
select
...
create policy "update own goals" on public.goals for
...
create policy "delete own goals" on public.goals for delete to authenticated using (
  user_id = (
    select
      auth.uid ()
  )
